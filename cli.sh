...
importConfig() {
    if [ ! -f "$ENV_FILE" ]; then
        ...
        echo "Are you in the correct folder to run this command?"
        exit 1
    fi
    source $ENV_FILE
}
...
runDockerCompose() {
    $DOCKER_COMPOSE_CMD -f docker-compose.yml -f $COMPOSE_FILE --env-file $ENV_FILE "$@"
}
...
createJackettPassword(){
    ...
    NODE_COMMAND="node -e \"const crypto = require('crypto'); const input = '${JACKETT_PASSWORD}${JACKETT_API_KEY}'; const hash = crypto.createHash('sha512'); hash.update(Buffer.from(input, 'utf16le')); console.log(hash.digest().toString('hex'));\""
    
    # Use sudo for docker if needed
    JACKETT_PASSWORD_HASH=$(sudo docker run --rm node:20-slim sh -c "$NODE_COMMAND")
